...
import decimal
import time
from .models import Patient, User, USER_ROLES, SERVICE_STATUS, Visits, BLOOD_TYPES, Report
    ...
    if request.user.is_authenticated:
        ...
        if user.is_active and user.is_staff:
            Served_patients = Patient.objects.filter(
                status='Served', visits__assigned_to=request.user).count()
...
def updatepatientprofileview(request, pk):
    ...
    photo = str(patient_obj.photo).split(
        '/')[-1] if patient_obj.photo else None
        ...
        if user.is_active and user.is_staff:
            if slug == '0':
                ...
                Patient_list = Patient.objects.filter(
                    visits__assigned_to=request.user)
            elif slug == 'Waiting':
                Patient_list = Patient.objects.filter(
                    visits__assigned_to=request.user, status='Waiting')
            elif slug == 'Inservice':
                Patient_list = Patient.objects.filter(
                    visits__assigned_to=request.user, status='Inservice')
            elif slug == 'Served':
                Patient_list = Patient.objects.filter(
                    visits__assigned_to=request.user, status='Served')
            return render(request, "patient/patients_list.html", {'patient_list': Patient_list})
        else:
            Patient_list = None
    return render(request, "patient/patients_list.html", {'patient_list': Patient_list})
...
class SearchPatientsView(ListView):
    ...
    def get_queryset(self):  # new
        ...
        patient_list = Patient.objects.filter(is_active=True).filter(
            Q(first_name__icontains=query) | Q(
                last_name__icontains=query) | Q(country__icontains=query)
        )
        return patient_list

class SearchPatientsProfileView(ListView):
    model = Patient
    ...
    def get_queryset(self):  # new
        ...
        staff_count = User.objects.count()
        Patients_count = Patient.objects.all().count()
        patient_list = Patient.objects.filter(is_active=True,visits__assigned_to=self.request.user).filter(Q(country__icontains=query))
        context = {    "patients_list": patient_list,
                       "Patients_count": Patients_count,
                       "staff_count": staff_count,
                       "Served_patients": Served_patients,
                       "Inservice_patients": Inservice_patients,
...
def Patient_inactive_view(request, pk):
    ...
    if patient.is_active:
        count = 0
        docs = Report.objects.filter(patient=patient)
        if docs:
            for doc in docs:
                if count == docs.count():
            ...
            patient.save()
    patient_list = Patient.objects.filter(is_active=True)
    return HttpResponseRedirect(reverse("viewpatient", kwargs={"patient_list": patient_obj.id}))
...
class ViewPDF(View):
    def get(self, request, pk, *args, **kwargs):
        ...
        pdf = render_to_pdf('report/pdf_report.html', context)
...
class DownloadPDF(View):
    def get(self, request, pk, *args, **kwargs):
        ...
        pdf = render_to_pdf('report/pdf_report.html', report)
        response = HttpResponse(pdf, content_type='application/pdf')
        filename = "report-{0}-{1}".format(medcare, report.date)
